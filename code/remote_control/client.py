...
class LoginScreen(QtWidgets.QDialog, Ui_Login_screen):   
	...
	def __init__(self):
		...
		self.setupUi(self)
		self.setWindowTitle("Log In - Smart Hmoe Client")
...
class RunningScreen(QtWidgets.QDialog, Ui_Running_screen):
	...
	def __init__(self):
		...
		self.setWindowTitle("Operation - SmartHome Client")
...
class CalibrateScreen(QtWidgets.QDialog, Ui_Calibrate_screen):
	...
	def calibration_show(self, calibration_status):
		...
		if self.calibration_status == 1:				# calibrate camera
			cali_action('camcali')
			self.setWindowTitle("Camera Calibration - SmartHome-V Client")
			...
			self.label_Info_2.setText("Use arrow keys or W, S keys.")
	...
	def keyPressEvent(self, event):
		...
		if key_press in (Qt.Key_Up, Qt.Key_W):    	# CLOSE
			if   self.calibration_status == 1:
				...
				pass
		elif key_press in (Qt.Key_Down, Qt.Key_S):	# OPEN
			if   self.calibration_status == 1:
...
class QueryImage:
	...
	Attributes:
		host, port. Port default 8081, post need to set when creat a new object

	"""
	def __init__(self, host, port=8081, argv="/?action=snapshot"):
